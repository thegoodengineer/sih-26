...
def pdhg_section(path: Path | None) -> str:
    ...
    out += [
        ...
        "`--option pdhg_stop_at_request=true` stops on the request alone and reports the "
        "point as `feasible`, never `optimal`. Measured on these instances at 1e-4 it costs "
        "0.85x the iterations (`bench/results/pdhg-stop-at-request-f18d4b0.csv`), and turns "
        "`share2b` from an iteration limit into a usable point at 807,760.",
        "",
