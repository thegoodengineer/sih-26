...
**31 checkable requirements across the five status tables above (the industrial-scope list is a domain list, not counted): 27 done, 3 partial, 1 not started** — since the last refresh, cutting planes moved from not started to done, off by default (#159), and presolve from partial to done (#92 merged as `780d7be`). The one row still not started is GPU acceleration (#16-#19).
