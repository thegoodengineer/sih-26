...
**Refreshed 2026-09-17 at `3b6ff92` (the #213 read), with the interior point's full Netlib
re-run at `fa7eb86` (#256) since the `59ac6e3` CSV that number previously cited was not a
commit on `main`. Every number cites the CSV it comes from, and the CSV it cites is the one
...
|---|---|---|
| Linear Programming | **done** | revised primal and dual simplex, the dual being the default LP engine since #165; Netlib full set 80/89 (`bench/results/netlib-full-e134aeb.csv`), medium 48/50, small 9/9 (`bench/results/netlib-{medium,small}-6bec31e.csv`; #34). Cross-checked against HiGHS (#154): on every instance where the solver produces a final answer, that answer agrees with HiGHS to 3.0e-07 or better - that worst case is `pilot`, and the next is `e226` at 3.1e-10, which is the figure this row used to quote as though it were the worst; 8 of the 12 instances cross-checked are cases where the published Netlib table is the outlier, not our answer — the full set is the headline per #142, the narrower tiers being row-capped and so the easier half |
...
| Interior-point methods | **done, opt-in** | #56 via #169: Mehrotra predictor-corrector on the bounded form, normal equations through a from-scratch sparse LDLᵀ (`src/ipm/ipm.cpp`, `src/la/ldl.cpp`), `--option algorithm=ipm`. Produces no basis, so it is not the node engine and not the default; on the full Netlib set, re-measured on `main` at `fa7eb86` after #193 (AMD ordering), #205, #229 and #241 all touched the interior point since the stale `59ac6e3` reading, it verifies 48/89 (`bench/results/netlib-full-fa7eb86-ipm.csv`), against 80/89 for the dual simplex at `e134aeb` (`bench/results/netlib-full-e134aeb.csv`, one commit earlier but no solver code changed between the two - #312). On the committed small set it is 9 of 9 verified on `main` at `e134aeb` (`bench/results/netlib-small-e134aeb-ipm.csv`, `--solver-option algorithm=ipm`); the tier tables in `docs/BENCHMARKS.md` ignore that file by design, since `latest_result.py` counts only default-option runs as a tier's evidence. |
