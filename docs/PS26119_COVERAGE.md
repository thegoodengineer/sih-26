...
**Refreshed 2026-09-17 at `3b6ff92` (the #213 read); every number cites the CSV it comes from, and the CSV it cites is the one the number came from. One exception is named where it sits: the interior point's full Netlib CSV was measured at `59ac6e3`, which is not a commit on `main` (#256).**

## Problem classes

| requirement | status | evidence / issue |
|---|---|---|
| Linear Programming | **done** | revised primal and dual simplex, the dual being the default LP engine since #165; Netlib full set 78/89 (`bench/results/netlib-full-2b4eb6b.csv`), medium 48/50, small 9/9 (`bench/results/netlib-{medium,small}-6bec31e.csv`; #34). Cross-checked against HiGHS (#154): on every instance where the solver produces a final answer, that answer agrees with HiGHS to 3.0e-07 or better - that worst case is `pilot`, and the next is `e226` at 3.1e-10, which is the figure this row used to quote as though it were the worst; 8 of the 12 instances cross-checked are cases where the published Netlib table is the outlier, not our answer — the full set is the headline per #142, the narrower tiers being row-capped and so the easier half |
...
| GPU acceleration | **not on `main`** | #16-#19; the CUDA backend is PR #274, open, not yet built or measured on a GPU, and being carved into #16 and #17 on the two team laptops that have one. The engine it needs — restarted PDHG — exists, is verified, and solves the 5000x5000 instance in the demo on CPU. `--gpu` warns and falls back. |
...
- PR #274, the CUDA backend for PDHG (GPU acceleration row). Open, not yet built or measured
  on a machine with a GPU, so the row stays not on `main`.

The four this section used to list (#137, #140, #141, #144) are merged, and the rows above
count them.

## Rough tally

**32 checkable requirements across the five status tables above (the industrial-scope list is a domain list, not counted): 28 done, 3 partial, 1 not on `main`** — sensitivity ranging (#220) is the row added since the previous count; before that, cutting planes moved to done, off by default (#159), and presolve from partial to done (#92 merged as `780d7be`). The one row not on `main` is GPU acceleration (#16-#19, PR #274).
...
- **GPU acceleration is in the PS title and is not on `main`** (#16-#19): PR #274 is open and not yet built or measured on a GPU. The CPU first-order
