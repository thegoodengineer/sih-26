...
The Windows C runtime and nothing else. `ldd` on the Linux build is generated by the
`provenance` job in `.github/workflows/ci.yml` on **every pull request**, and that job fails
the build if any linked library name matches a known solver.
