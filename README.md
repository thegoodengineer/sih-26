...
| **Cutting planes by default** | Root Gomory mixed-integer, lifted knapsack cover and mixed-integer rounding cuts exist (#159, #221; `--option enable_root_cuts=true`) and are off by default. On the 30-instance MIPLIB set at 60 s on `main` at `0d33665` they cost no proof and no published match (14 reached, 9 proved either way) and take the node count to 0.912x (`bench/results/miplib-cuts-{off,on}.csv`; `docs/BENCHMARKS.md` section 2, with the root gap closed per instance). The default is decided together with the cut rounds below the root (#221). Branch and bound itself has reliability branching (#69) and warm-started dual node LPs (#65). This is why MIPLIB proves few optima. |
