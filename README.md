...
| 4 | Restarted PDHG — **CPU done**, its answer finished by the interior point by default since #229; CUDA backend not on `main`: the CUDA backend is PR #153, open, and has never been built or measured here (no GPU on the development machine) | partial |
...
src/pdhg          restarted PDHG, CPU; its answer is finished by the interior point (#229)
                                                       (CUDA backend: not on main, PR #153)
...
| **Interior point: stop before the barrier breaks the factorization** | [#209](https://github.com/thegoodengineer/sih-26/issues/209) | #241 reads the regularization spike as convergence and stops one factorization earlier. Re-measured on `main` at `54fcb6f` (`bench/results/scale-staircase-54fcb6f.csv`): 26 iterations, relative error 1.4e-08, no NaN step, status `feasible` - not `optimal`, because its dual side misses the 1e-7 tolerance, which crossover (#219) is the honest way to close. The interior point's small Netlib set stays 9 of 9 on the same commit. |

### Finish

| what | where it stands |
|---|---|
| **A measurement pass on `main`** | The staircase family is re-measured at `54fcb6f` for #209. The random and refinery families (the refinery at its documented 12 / 365 / 8,760 periods) and Mittelmann still have to be re-measured on `main` after #240 and #242, committed to `bench/results/`, and `docs/BENCHMARKS.md` regenerated from them; that closes the last acceptance box of #208 and #210. It needs the machine alone for about an hour, on mains, and both attempts so far died with the session that launched them. |
