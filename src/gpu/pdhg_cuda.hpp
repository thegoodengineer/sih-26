...
/// Create a CUDA context and upload the frozen CSC matrix, constructing CSR representation on
/// GPU.
[[nodiscard]] PdhgCudaContext* pdhg_cuda_create(std::size_t rows, std::size_t cols,
                                                std::size_t nnz,
...
[[nodiscard]] bool pdhg_cuda_upload_problem_data(PdhgCudaContext* context, const double* cost,
...
[[nodiscard]] bool pdhg_cuda_upload_x(PdhgCudaContext* context, const double* host,
                                      std::size_t size);

/// Upload the initial dual vector.
[[nodiscard]] bool pdhg_cuda_upload_y(PdhgCudaContext* context, const double* host,
                                      std::size_t size);
...
[[nodiscard]] bool pdhg_cuda_step(PdhgCudaContext* context, double tau, double sigma,
                                  double omega, double* movement, double* interaction);
...
[[nodiscard]] bool pdhg_cuda_step_batch(PdhgCudaContext* context, double eta, double omega,
                                        double spectral_norm, std::int64_t iteration,
                                        std::int64_t averaged, std::int64_t iteration_limit,
                                        int max_accepted, PdhgBatchResult* result);

/// Accept and commit the candidate step on the GPU (x <- x_next, y <- y_next)
/// and accumulate running sum vectors for averaging.
[[nodiscard]] bool pdhg_cuda_accept_step(PdhgCudaContext* context);

/// Download the running average vectors (x_sum / count, y_sum / count).
[[nodiscard]] bool pdhg_cuda_download_average(PdhgCudaContext* context, double* host_x_avg,
                                              double* host_y_avg, std::size_t count);

/// Reset running average sum vectors on the GPU (used on restart).
[[nodiscard]] bool pdhg_cuda_reset_sum(PdhgCudaContext* context);

/// Download the current primal vector.
[[nodiscard]] bool pdhg_cuda_download_x(PdhgCudaContext* context, double* host,
                                        std::size_t size);

/// Download the current dual vector.
[[nodiscard]] bool pdhg_cuda_download_y(PdhgCudaContext* context, double* host,
                                        std::size_t size);
