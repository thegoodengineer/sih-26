...
void polish_with_the_interior_point(Solution* first, const Model& model, const Options& options,
  ...
  Solution polished = ipm::solve_ipm(model, polish, logger, &warm, control);
  ...
  if (problem_class == ProblemClass::kLp) {
    ...
    const auto run_lp_engine = [&](const Model& target) -> Solution {
      if (want_pdhg) {
        ...
        polish_with_the_interior_point(&first, target, options, logger, timer, control);
        return first;
      }
      return want_ipm    ? ipm::solve_ipm(target, options, logger, nullptr, control)
             : want_dual ? solve_dual_simplex(target, options, logger, nullptr, control)
